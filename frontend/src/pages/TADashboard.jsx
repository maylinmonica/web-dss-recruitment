                      ...
                      {applicants.map((app) => (
                        <tr key={app.id} className="hover:bg-slate-50/50 transition-colors duration-150">
                          ...
                          <td className="px-6 py-4 text-right">
                            <button onClick={() => openAuditWorkspace(app)} className="inline-flex items-center gap-1.5 px-3.5 py-1.5 bg-slate-950 text-white hover:bg-sky-600 rounded-full text-[11px] font-bold uppercase tracking-wider transition-all shadow-sm">{app.status === 'Verified' ? 'Tinjau Ulang' : 'Mulai Audit'}<ArrowRight className="w-3 h-3" /></button>
                          </td>
                        </tr>
                      ))}
                    </tbody>
                ...
                <div className="bg-white rounded-2xl border border-slate-200/60 p-5 shadow-sm">
                  <form onSubmit={handleVerifyScores} className="space-y-4">
                    ...
                    <div className="pt-3 border-t border-slate-100 flex items-center justify-end gap-2.5">
                      ...
                      <button type="submit" disabled={submitLoading} className="px-5 py-2 bg-slate-950 hover:bg-sky-600 text-white rounded-full text-xs font-bold uppercase tracking-wide transition-all shadow-sm flex items-center gap-1.5">
