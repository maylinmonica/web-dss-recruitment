    ...
    <div className="min-h-screen bg-[#F5F7FB] flex flex-col md:flex-row text-slate-800 font-sans antialiased selection:bg-sky-100 selection:text-slate-900">
      ...
      <div className="flex-1 flex flex-col overflow-y-auto">
        <main className="main-workspace-container flex-1 p-6 sm:p-10 space-y-8 relative z-10 animate-in fade-in duration-150">
          ...
          {/* SINKRONISASI BARIS FILTER DAN TOMBOL REFRESH */}
                        ...
                        <tr key={i} className="hover:bg-slate-50/50 transition-colors">
                          ...
                          {COL_KEYS.map(k => (
                            <td key={k} className="p-4 font-mono text-slate-600">
                              {row.values[k] !== undefined ? parseFloat(row.values[k]).toFixed(2) : '0.00'}
                            </td>
                          ))}
                        </tr>
                      ))}
                    ...
                    <tbody className="divide-y divide-slate-100 text-slate-700">
                      {ranking.map((r, idx) => (
                        <tr key={r.id} className="hover:bg-slate-50/50 transition-colors">
                          <td className="p-4 font-mono font-bold text-slate-900">#{idx+1}</td>
                          <td className="p-4 font-semibold text-slate-900">{r.name}</td>
                          <td className="p-4 font-mono text-rose-600 font-medium">{r.d_plus}</td>
                          <td className="p-4 font-mono text-emerald-600 font-medium">{r.d_minus}</td>
                          <td className="p-4">
